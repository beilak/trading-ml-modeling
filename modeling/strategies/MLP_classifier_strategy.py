import pandas as pd
from MLPClassifier_modelling import build_model, select_feature_columns, select_target_columns, train_model_MLPClassifier
...
class MLPClassifierStrategy(BaseStrategy):
    ...
    def model(self, df: pd.DataFrame):
        ...
        print(f"\nОбучаем MLPClassifier на {len(train_df)} строк (до {last_date.date()})")
        ...
        model = train_model_MLPClassifier(X, y)
    ...
    def generate_signals(self, current_date: pd.Timestamp, tickers: list) -> pd.Series:
        """
        Генерация торговых сигналов на основе обученной MLP-модели.
        """
        try:
